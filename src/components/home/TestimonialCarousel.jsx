				...
				<Swiper
					...
					pagination={{
						...
						renderBullet: (index, className) =>
							`<span class="${className} w-2 h-2 rounded-full bg-gray-300 inline-block transition-all duration-300 mx-1"></span>`,
					}}
					onBeforeInit={(swiper) => {
						// Attach custom navigation refs before swiper init
					...
					{testimonials.map((t, idx) => (
						<SwiperSlide key={idx}>
							{({ isActive }) => (
								<div
									className={`relative rounded-[2rem] p-6 transition-all duration-500 pb-20 pt-20 ${
										isActive
